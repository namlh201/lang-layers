#!/usr/bin/env python3
"""Visualise Chinese-probability heatmap across layers and token positions.

For each token position *i* and layer *j*, computes the probability that the
model's prediction at that layer/position is Chinese (cmn_Hans), using top-K
marginalisation over the lens predictions weighted by per-token Chinese
probabilities from the token language distribution:

    P_cmn(i, j) = sum_k P(cmn | t_k) * p_lens(t_k | h)

computed in log-space via logsumexp for numerical stability:

    log P_cmn(i, j) = logsumexp_k [ log P(cmn | t_k) + log p_lens(t_k) ]
...
Usage::
    ...
    python visualize_chinese_prob.py \\
    ...
    python visualize_chinese_prob.py lens.json --lang-dist dist.json --output cmn_heatmap.png
"""
...
def build_cmn_prob_map(
    lang_dist: dict, tokenizer,
) -> tuple[dict[str, float], int, dict[str, str]]:
    """Build decoded_token_str -> P(cmn | token) map from the lang dist JSON.
    ...
    P(cmn | token). When multiple token_ids decode to the same string,
    ...
    # Phase 1: Collect (decoded_str, cmn_prob, total_count) for each token_id
    str_to_cmns: dict[str, list[tuple[float, float]]] = {}
    decoded_to_raw: dict[str, str] = {}
    for tid_str, entry in tokens.items():
        tid = int(tid_str)
        langs = entry.get("langs", {})
        cmn = langs.get("cmn_Hans", 0.0)
        total_count = entry.get("total_count", 0)
        decoded = tokenizer.decode([tid])
        str_to_cmns.setdefault(decoded, []).append((cmn, total_count))
    ...
    str_to_cmn: dict[str, float] = {}
    for decoded, cmn_count_list in str_to_cmns.items():
        total_weight = sum(tc for _, tc in cmn_count_list)
        if total_weight > 0:
            weighted_cmn = sum(cmn * tc for cmn, tc in cmn_count_list) / total_weight
        else:
            weighted_cmn = sum(cmn for cmn, _ in cmn_count_list) / len(cmn_count_list)
        str_to_cmn[decoded] = weighted_cmn
    ...
    return str_to_cmn, num_langs, decoded_to_raw


def lookup_cmn_logprob(
    token_str: str,
    str_to_cmn: dict[str, float],
    uniform_log: float,
    _cache: dict[str, float],
) -> float:
    """Look up log P(cmn | token_str) from the decode-string lookup table.

    Returns log P(cmn | t) if found, otherwise the uniform log-prior (1/M).
    Caches results per token string for efficiency.
    """
    if token_str in _cache:
        return _cache[token_str]

    cmn = str_to_cmn.get(token_str)
    if cmn is not None:
        log_p = np.log(cmn) if cmn > 0 else -np.inf
    else:
        log_p = uniform_log
...
def compute_cmn_prob_matrix(
    ...
    """Compute the Chinese-probability matrix.

    Returns:
        layer_names:  e.g. ["embed", "layer 0", ...]
        token_labels: e.g. ["Ace", "hn", ...]
        cmn_probs:    (n_tokens, n_layers) array of P(cmn) in [0, 1]
    ...
    cmn_probs = np.zeros((n_tokens, n_layers), dtype=np.float64)
    ...
    for i, tok_data in enumerate(tokens):
        ...
        for j, layer_entry in enumerate(tok_data["layers"]):
            ...
            if not top_preds:
                ...
                if top1 and prob > 0:
                    log_cmn = lookup_cmn_logprob(
                        top1, str_to_cmn, uniform_log, _cache,
                    )
                    cmn_probs[i, j] = np.exp(log_cmn + np.log(prob))
                else:
                    cmn_probs[i, j] = 0.0
                continue
            ...
            if top_1_only:
                pred = top_preds[0]
                log_cmn = lookup_cmn_logprob(
                    pred["token"], str_to_cmn, uniform_log, _cache,
                )
                log_prob = np.log(pred["prob"]) if pred["prob"] > 0 else -np.inf
                cmn_probs[i, j] = np.exp(log_cmn + log_prob)
                continue
            ...
            for k, pred in enumerate(top_preds):
                ...
                log_prob = np.log(pred["prob"]) if pred["prob"] > 0 else -np.inf
                log_terms[k] = log_cmn + log_prob

            cmn_probs[i, j] = np.exp(logsumexp(log_terms))

    np.clip(cmn_probs, 0.0, 1.0, out=cmn_probs)

    return layer_names, token_labels, cmn_probs, top1_tokens
...
def plot_heatmap(
    ...
    probs_t = cmn_probs.T  # (n_layers, n_tokens)
    ...
    ax.set_ylabel("Layer")
    ax.set_title("Chinese Probability Across Layers")
...
def plot_layer_avg(
    ...
    n_layers = cmn_probs.shape[1]
    layer_avg = cmn_probs.mean(axis=0)
    ...
    ax.set_ylim(0, y_max)
    ax.set_ylabel("Mean P(Chinese)")
    ax.set_xlabel("Layer")
    ax.set_title("Mean Chinese Probability per Layer")
...
def main():
    parser = argparse.ArgumentParser(
        description="Heatmap of P(Chinese) across layers and token positions.",
    )
    parser.add_argument(
        "input",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output prefix. Four files will be created: <prefix>_cmn.png, <prefix>_cmn_line.png, "
             "<prefix>_top1_cmn.png, <prefix>_top1_cmn_line.png (default: show interactively).",
    )
    parser.add_argument(
        "--cmap",
        type=str,
    ...
    str_to_cmn, num_langs, decoded_to_raw = build_cmn_prob_map(lang_dist, tokenizer)
    print(f"  {len(str_to_cmn)} unique decoded token strings in lang dist, {num_langs} languages",
          file=sys.stderr)
    ...
    for top_1_only, suffix in [(False, ""), (True, "top1_")]:
        ...
        print(f"\n--- Computing [{mode_label}] Chinese probability matrix ---", file=sys.stderr)

        layer_names, token_labels, cmn_probs, top1_tokens = compute_cmn_prob_matrix(
            lens_data, str_to_cmn, num_langs, args.top_k,
        ...
        if args.output:
            output_path = f"{args.output}_{suffix}cmn.png"
            line_path = f"{args.output}_{suffix}cmn_line.png"
        else:
            output_path = None
            line_path = None

        plot_heatmap(
            layer_names, token_labels, cmn_probs, top1_tokens,
