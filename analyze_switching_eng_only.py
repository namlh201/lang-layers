...
def load_all_files(lens_dir: Path | None = None):
    ...
    files = sorted(
        ...
        and _is_english_source_dir(Path(f).parent.name)
    )
    results = []
    for fpath in files:
...
def main():
    ...
    if model_name_override:
        # --model: single model dir, skip multi-model detection
        output_dir = (
            Path(args.output_dir) if args.output_dir else lens_dir.parent
        )
        output_dir.mkdir(parents=True, exist_ok=True)
        suffix = (
            "_eng_src_only_langdist" if lang_dist_path else "_eng_src_only"
        )
        output_path = (
            output_dir / f"{model_name_override}_switching{suffix}.txt"
        )
    ...
    elif lens_dir.is_dir() and any(lens_dir.iterdir()):
        children = sorted(d for d in lens_dir.iterdir() if d.is_dir())
        model_dirs = [c for c in children if any(c.rglob("*.json"))]
        if model_dirs:
            output_dir = (
                Path(args.output_dir) if args.output_dir else lens_dir
            )
            output_dir.mkdir(parents=True, exist_ok=True)
            for model_dir in model_dirs:
                model_name = model_dir.name
                suffix = (
                    "_eng_src_only_langdist"
                    if lang_dist_path
                    else "_eng_src_only"
                )
                output_path = (
                    output_dir
                    / f"{model_name}_switching{suffix}.txt"
                )
                print(f"\nModel: {model_name}", file=sys.stderr)
            ...
            # Single model dir (no subdirs)
            model_name = lens_dir.name
            output_dir = (
                Path(args.output_dir) if args.output_dir else lens_dir.parent
            )
            output_dir.mkdir(parents=True, exist_ok=True)
            ...
            output_path = (
                output_dir / f"{model_name}_switching{suffix}.txt"
            )
            print(f"\nOutput: {output_path}", file=sys.stderr)
            with open(output_path, "w") as out_f:
                old_stdout = sys.stdout
